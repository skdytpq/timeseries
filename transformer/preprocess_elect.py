...
def gen_covariates(times, num_covariates):
    ...
    for i, input_time in enumerate(times):
        ...
        # A Signal A  Feature  : Zscore 출력을 한다. 전체 데이터 분포에서 얘가 얼마나 평균에 벗어나 있는지 알 수 있잖아
        # train dataset shape[0]이 같아진다. 
        # 시간 축이 있고 시간에 시그널 변화 특정 시점에서는 특정 시그널이 어떻게 변한다, 포착하기 위해서 zscore 구한다. 
        # zscore : 전체 데이터 셋에서 구한다.
        # 공변량 : 전체 데이터에서 한번 구함, 윈도우 사이즈에 대해서도 한번 구함 
        # 윈도우 사이즈 변화에 따라서 어떻게 통계 값들이 달라지는지 
